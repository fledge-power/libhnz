...
void VoieHNZ::iConnecteSlave() {
    // Si on a une classe d'attente associe, c'est elle qui fait le CONNECT  (ESCLAVE)
    //if (m_pVoiePendante) return m_pVoiePendante->iConnecte();
    //return socketTcp.iTCPConnecte() ;
    connslave->iTCPConnecteServeur();
}

void VoieHNZ::setServer(TcpConnexion *server) {
    connslave = server;
}

void VoieHNZ::setClient(TcpConnexion *client) {
    connMaster = client;
}

void VoieHNZ::iConnecteMaster() {
    connMaster->iTCPConnecteClient();
}
...
void  VoieHNZ::vInitEnteteTrameData( MSG_TRAME *pTrame, bool bMaj ) {
    ...
    if ( !bMaj ) {
        // Pour une nouvelle trame, il faut laisser la place pour les 2 premiers octets
    ...
    //vLogMessageVoie( DUMP_DEBUG, TypeTrace::Autre, 0, "HNZ : Envoi Trame I - NR=%d - NS=%d", m_iNoTrameAttEmission, m_iNoSequenceEnCours ) ;
...
void VoieHNZ::vEnvoiTrameVersHnz(MSG_TRAME * pTrame) {
    ...
    // Si il n'y a pas eu d'mission rcente, on envoie, sinon, un timer dclenchera l'mission plus tard.
...
void VoieHNZ::vEnvoiTrameVersHnz() {
    ...
    for (int i=0; i< (int) pTrame->usLgBuffer; i++) {
        ...
        else bTrame[iIndex++] = pTrame->aubTrame[i];
    }
    ...
    m_AutomateHNZ->vAddTimer(2000 /*délai intertrame*/, /*EVT_EMISSION*/ 1, NULL);
...
bool VoieHNZ::bAjouteCar(unsigned char ub) {
    ...
    if (m_bTransparenceEnCours) {
        if (ub == DLE_HNZ) ub = DLE_HNZ;
        else if (ub == SUB_HNZ) ub = SEND_HNZ;
        else {
            ub = 0;
        }
        m_bTransparenceEnCours = false;
...
VCRC VoieHNZ::calculCRCMot(MSG_TRAME *pTrame, unsigned int uiPolynome) {
    ...
    for ( jw = 2 ; jw < iLg - 2 ; jw ++ ) {
        ...
        for (iw = 0 ; iw < 8 ; iw ++) {
            if ((vcrc.uiVal & 0x00000001) != 0) vcrc.uiVal = (vcrc.uiVal >> 1) ^ uiPolynome ;
            else                                vcrc.uiVal = (vcrc.uiVal >> 1) ;
        }
    }
...
\return Difference entre CRC calcule et CRC present dans la trame (soit '0' si identiques)
****************************************************************************************/
int VoieHNZ::iVerifCRCMot(MSG_TRAME *pTrame, unsigned int uiPolynome) {
    ...
    if ((vcrc.bVal[0] == pTrame->aubTrame[iLg-2]) && (vcrc.bVal[1] == pTrame->aubTrame[iLg-1]) ) {
        iRet = 1 ;
    } else {
        vcrc.usVal[0] = ~vcrc.usVal[0] ;
        if ((vcrc.bVal[0] != pTrame->aubTrame[iLg-2]) || (vcrc.bVal[1] != pTrame->aubTrame[iLg-1])) 	iRet = -1 ;
    }
    return iRet;
}
...
\n Le tampon 'trame' doit avoir la taille suffisante pour recevoir le CRC (2 octets)
****************************************************************************************/
...
bool VoieHNZ::vTraiteEvt(int iEvt) {
    ...
    if (iEvt == EVT_VOIE_TRAME_HNZ) {
       ...
       // m_pLigneHNZ->vEvtStats(EVT_STAT_RECEPTION_TRAME_A + (m_iVoie % NB_EQUIPEMENT_VOIES)) ;
    }
...
MSG_TRAME* VoieHNZ::trDepileTrame() {
    return m_ffTramesRecus.pDepile();
}